    ...
    for v in range(len(list_vel[:-1])):
        if list_vel[v] >= stop_vel:
            ...
            if list_vel[v+1] < stop_vel:
                ...
                mean_delta_temp.append(np.abs(np.mean(walk_temperatures) - T_ideal))
                duration_walk = 0
                walk_temperatures = []
            elif v == len(list_vel[:-2]):
                all_walk_durations.append(duration_walk)
                mean_delta_temp.append(np.abs(np.mean(walk_temperatures) - T_ideal))
    return all_walk_durations, mean_delta_temp
...
ALL_TEMPS_at_walk = []
for testbee in head:
    walking_times, mean_temps = calc_dur(testbee)
    ALL_WALKS_at_dT.extend(walking_times)
    ALL_TEMPS_at_walk.extend(mean_temps)
#plt.hist(ALL_WALKS_at_dT, bins=100)
plt.scatter(ALL_TEMPS_at_walk, ALL_WALKS_at_dT)
plt.xlabel("delta T")
