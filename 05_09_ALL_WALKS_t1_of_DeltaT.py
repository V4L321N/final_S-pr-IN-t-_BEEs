    ...
    for v in range(len(list_vel[:-1])):
        if list_vel[v] >= stop_vel:
            ...
            if list_vel[v+1] < stop_vel:
                walk_durations.append(duration_walk)
                duration_walk = 0
            elif v == len(list_vel[:-2]):
                walk_durations.append(duration_walk)
    return walk_durations
...
for testbee in head:
    ALL_walk_durations.extend(calc_dur(testbee))

plt.xlabel('walking times [s]')
plt.ylabel('counts')
plt.hist(ALL_walk_durations, bins=100, range=(0,100))#, density=True)
plt.show()
