...
# def theta_FFT(testbee):
#     return np.fft.fft(recalc_w_2pi(testbee), norm="ortho") #either norm="ortho" here or divided by C_1 in PSD()
# #plt.plot(theta_FFT)
...
def PSD(item):
    theta_FFT = np.fft.fft(recalc_w_2pi(item), norm="ortho")
    PSD_list = []
    for i in range(round(len(theta_FFT/2))):
        C_1 = 1#/len(theta_FFT)
        absolute = C_1 * (theta_FFT[i] * np.conj(theta_FFT[i]))
...
for testbee in head:
    ...
    popt, pcov = curve_fit(S, omega, psd_, bounds=([0, 0], [10000, 10]))
    print(popt)
    plt.plot(omega, PSD(testbee))
